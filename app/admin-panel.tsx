...
import { Bytes, addDoc, collection, deleteDoc, doc, getDoc, getDocs, limit, orderBy, query, serverTimestamp, setDoc } from "firebase/firestore";
...
export default function AdminPanel({ user, onNotice }: Props) {
  ...
  const loadAdmin = useCallback(async () => {
    ...
    try {
      ...
      setPdfInfo(metadata.exists() ? `${String(metadata.data().fileName || "Caderno SAME")} · ${(Number(metadata.data().size) / 1024 / 1024).toFixed(1)} MB · ${Number(metadata.data().chunkCount)} partes protegidas` : "Nenhum PDF protegido enviado ainda.");
    } catch {
  ...
  async function uploadPdf(file?: File) {
    ...
    let uploadedVersion = ""; let uploadedChunks = 0;
    try {
      ...
      if (new TextDecoder("ascii").decode(fileBytes.slice(0, 5)) !== "%PDF-") return onNotice("O arquivo escolhido não é um PDF válido.");
      const version = `v${Date.now()}`; const chunkCount = Math.ceil(fileBytes.length / PDF_CHUNK_SIZE); uploadedVersion = version;
      for (let index = 0; index < chunkCount; index += 1) { const chunk = fileBytes.slice(index * PDF_CHUNK_SIZE, Math.min((index + 1) * PDF_CHUNK_SIZE, fileBytes.length)); await setDoc(doc(firestore, PROTECTED_PDF_CHUNKS, `${version}-${String(index).padStart(3, "0")}`), { version, index, bytes: Bytes.fromUint8Array(chunk) }); uploadedChunks = index + 1; setUploadProgress(Math.round(((index + 1) / chunkCount) * 100)); }
    ...
    } catch { if (uploadedVersion) for (let index = 0; index < uploadedChunks; index += 1) await deleteDoc(doc(firestore, PROTECTED_PDF_CHUNKS, `${uploadedVersion}-${String(index).padStart(3, "0")}`)).catch(() => undefined); onNotice("O upload foi interrompido e as partes incompletas foram limpas. Confira as regras do Firestore."); }
