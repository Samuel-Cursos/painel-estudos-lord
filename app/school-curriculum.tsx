...
export default function SchoolCurriculum({ year, progress, onCompleteLesson }: Props) {
  ...
  return <section className="grade-curriculum">
    ...
    {activeLesson && <div className="modal-backdrop lesson-reader-backdrop" role="presentation" onMouseDown={() => setActiveLesson(null)}><article className="school-lesson-reader" role="dialog" aria-modal="true" aria-label={`Aula de ${activeLesson.topic}`} onMouseDown={(event) => event.stopPropagation()} style={{ "--accent": schoolSubjectMeta[activeLesson.subject].color } as React.CSSProperties}>
      ...
      <div className="reader-progress"><i /><i /><i /><i /><span>ESTUDO GUIADO</span></div>
      <section className="reader-block"><span className="reader-label">1 · APRENDA</span><h3>Explicação da aula</h3>{activeLesson.reading.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}</section>
      <section className="reader-example"><span className="reader-label">2 · VEJA FUNCIONAR</span><h3>{activeLesson.exampleTitle}</h3><p>{activeLesson.example}</p></section>
      <section className="reader-keypoints notebook-card"><div><span className="reader-label">3 · ANOTE NO CADERNO</span><h3>Resumo essencial</h3><p>Copie estes pontos. Escrever ajuda a organizar e lembrar o conteúdo.</p></div><ul>{activeLesson.keyPoints.map((point, index) => <li key={point}><span>{index + 1}</span>{point}</li>)}</ul></section>
      <section className="reader-practice"><span className="reader-label">4 · CONFIRA COM SUAS PALAVRAS</span><h3>Antes do teste</h3>{activeLesson.guidedPractice.map((step, index) => <div key={step}><span>{index + 1}</span><p>{step}</p></div>)}</section>
      <section className="reader-warning"><span>!</span><div><strong>Erro comum</strong><p>{activeLesson.commonMistake}</p></div></section>
      <section className="reader-exit"><span className="reader-label">5 · DECIDA</span><h3>Você entendeu esta aula?</h3><p>{activeLesson.exitTicket}</p><div className="reader-understanding-actions"><button className="secondary" onClick={() => setReaderMode("help")}>Ainda não entendi</button><button className="primary" onClick={() => resetReader("quiz")}>Entendi, testar agora →</button></div></section>

      {readerMode === "help" && <section className="reader-help-panel"><span className="reader-label">EXPLICAÇÃO DE OUTRO JEITO</span><h3>Vamos com mais calma.</h3>{activeLesson.alternateReading.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}<div className="reader-help-actions"><a className="reader-video-link" href={activeLesson.videoSearchUrl} target="_blank" rel="noreferrer">▶ Procurar videoaula sobre {activeLesson.topic}</a><button className="primary" onClick={() => resetReader("quiz")}>Agora entendi, testar →</button></div></section>}

      {readerMode === "quiz" && <section className="reader-quiz"><span className="reader-label">TESTE DA AULA</span><h3>{activeLesson.quiz.prompt}</h3><p>Escolha uma alternativa. A próxima aula só será liberada depois do acerto.</p><div className="quiz-options">{activeLesson.quiz.options.map((option, index) => <button key={option} className={selectedOption === index ? "selected" : ""} onClick={() => { setSelectedOption(index); setWrongAnswer(false); }}><span>{String.fromCharCode(65 + index)}</span>{option}</button>)}</div>{wrongAnswer && <div className="quiz-feedback wrong"><strong>Ainda não foi dessa vez.</strong><p>{activeLesson.quiz.explanation}</p><button onClick={() => setReaderMode("help")}>Rever com outra explicação</button></div>}<button className="primary quiz-submit" disabled={selectedOption === null} onClick={() => checkAnswer(activeLesson)}>Conferir resposta</button></section>}

      {readerMode === "passed" && <section className="reader-passed"><span>✓</span><div><strong>Você acertou e concluiu a aula.</strong><p>{activeLesson.quiz.explanation}</p></div></section>}
