...
type EssayRecord = { id: number; theme: string; date: string; score: string; competency: string; nextStep: string };
type SyncState = "offline" | "syncing" | "synced" | "error";
type CloudDashboard = { progress?: ProgressMap; skillProgress?: SkillProgress; assessmentResults?: AssessmentMap; questionProgress?: QuestionProgressMap; tasks?: Task[]; essays?: EssayRecord[] };
...
function displaySavedDate(value: string) {
...
export default function StudyDashboard() {
  ...
  const [essayCompetency, setEssayCompetency] = useState("Competência 1 · Norma-padrão");
  const [essayNextStep, setEssayNextStep] = useState("");
  ...
  function updateAssessment(name: string, field: keyof AssessmentRecord, value: string) {
    ...
    const cleanValue = field === "date" ? value : field === "time" ? value.replace(/[^0-9hm: ]/gi, "").slice(0, 6) : digits ? String(Math.min(45, Number(digits))) : "";
  ...
  function addEssay(event: FormEvent) { event.preventDefault(); if (!essayTheme.trim()) return setNotice("Digite o tema da redação."); if (essayScore && Number(essayScore) > 1000) return setNotice("A nota da redação deve ficar entre 0 e 1000."); const next: EssayRecord[] = [{ id: Date.now(), theme: essayTheme.trim(), date: localDateKey(), score: essayScore, competency: essayCompetency, nextStep: essayNextStep.trim() }, ...essays]; setEssays(next); window.localStorage.setItem("clareia-essays", JSON.stringify(next)); void saveToCloud({ essays: next }); setEssayTheme(""); setEssayScore(""); setEssayNextStep(""); setNotice("Redação registrada no seu histórico."); }
  function deleteEssay(essay: EssayRecord) { if (!window.confirm(`Excluir o registro da redação “${essay.theme}”?`)) return; const next = essays.filter((item) => item.id !== essay.id); setEssays(next); window.localStorage.setItem("clareia-essays", JSON.stringify(next)); void saveToCloud({ essays: next }); setNotice("Registro de redação excluído."); }
  ...
  return <main className="app-shell">
    ...
    <section className="workspace"><header className="topbar"><button className="menu-button" aria-label="Abrir menu" onClick={() => setMobileMenu((value) => !value)}>☰</button><div><p>{formatDate(currentDate)}</p><h1>{viewLabels[view]}</h1></div><div className="top-actions"><div className="academic-year-chip"><span>ENEM</span><small>{currentDate?.getFullYear() ?? ""}</small></div><button className="quick-add" onClick={() => setView("tasks")}>+ Planejar</button><button className={`sync-account ${syncState}`} onClick={disconnectGoogle} title="Clique para sair da conta"><span>{syncState === "syncing" ? "↻" : syncState === "error" ? "!" : "✓"}</span><div><strong>{syncState === "syncing" ? "Salvando..." : syncState === "error" ? "Só neste aparelho" : "Sincronizado"}</strong><small>{user.displayName?.split(" ")[0] ?? user.email}</small></div></button></div></header>
      ...
      {view === "bank" && <div className="page-content enem-page"><section className="bank-focus-head"><span className="eyebrow">PRÁTICA ENEM</span><h2>1.000 questões reais + treino com correção</h2><p>O caderno reúne Matemática e Ciências da Natureza; o banco rápido complementa Linguagens, Humanas e Inglês.</p></section><div className="enem-bank-wrap"><QuestionBank key={`${user.uid}-${questionFocus?.nonce ?? 0}`} schoolYear="3em" progress={questionProgress} focus={questionFocus} user={user} pdfAllowed={pdfAllowed} pdfEnabled={appSettings.pdfEnabled} publicPracticeEnabled={appSettings.publicPracticeEnabled} dailyQuestionGoal={appSettings.dailyQuestionGoal} onProgressChange={updateQuestionProgress} onNotice={setNotice} /></div></div>}
      {view === "writing" && <div className="page-content writing-page"><section className="writing-hero"><div><span className="eyebrow">REDAÇÃO ENEM · 5 COMPETÊNCIAS</span><h2>Escreva, corrija e transforme erro em estratégia.</h2><p>Registre cada produção, acompanhe a nota e escolha uma única melhoria para a próxima redação.</p></div><div><strong>{essays.length}</strong><span>redações registradas</span><strong>{essays.length ? Math.round(essays.reduce((sum, item) => sum + (Number(item.score) || 0), 0) / essays.length) : 0}</strong><span>média atual</span></div></section><div className="writing-layout"><form className="writing-form section-block" onSubmit={addEssay}><span className="eyebrow">NOVA PRODUÇÃO</span><h2>Registrar redação</h2><label><span>Tema</span><input maxLength={180} value={essayTheme} onChange={(event) => setEssayTheme(event.target.value)} placeholder="Ex.: desafios da inclusão digital no Brasil" /></label><div className="form-grid"><label><span>Nota (0–1000)</span><input inputMode="numeric" value={essayScore} onChange={(event) => { const value = event.target.value.replace(/\D/g, "").slice(0, 4); setEssayScore(value ? String(Math.min(1000, Number(value))) : ""); }} placeholder="Ex.: 760" /></label><label><span>Principal competência a melhorar</span><select value={essayCompetency} onChange={(event) => setEssayCompetency(event.target.value)}><option>Competência 1 · Norma-padrão</option><option>Competência 2 · Tema e repertório</option><option>Competência 3 · Argumentação</option><option>Competência 4 · Coesão</option><option>Competência 5 · Intervenção</option></select></label></div><label><span>Próximo passo</span><textarea maxLength={500} value={essayNextStep} onChange={(event) => setEssayNextStep(event.target.value)} placeholder="Uma ação concreta para melhorar na próxima redação" /></label><button className="primary" type="submit">Guardar no histórico</button></form><section className="section-block"><div className="section-heading"><div><span className="eyebrow">EVOLUÇÃO</span><h2>Histórico de redações</h2></div></div><div className="essay-history">{essays.map((essay) => <article key={essay.id}><div><strong>{essay.score || "—"}</strong><small>pontos</small></div><span><b>{essay.theme}</b><small>{essay.competency} · {displaySavedDate(essay.date)}</small><p>{essay.nextStep || "Sem próximo passo registrado."}</p></span><button aria-label={`Excluir redação ${essay.theme}`} onClick={() => deleteEssay(essay)}>Excluir</button></article>)}{!essays.length && <div className="empty-list large"><span>R</span><p>Sua primeira redação aparecerá aqui.</p></div>}</div></section></div></div>}
