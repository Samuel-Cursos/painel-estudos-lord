...
import { extendedEnglishCourse } from "./english-extended-data";
...
import { skillMasteryQuestion } from "./mastery-data";
...
// English drafts stay in their UID-scoped store, never in legacy shared caches.
const onlyEnemSkills = (value: SkillProgress): SkillProgress => Object.fromEntries(Object.entries(value).filter(([key]) => key !== "english-course-v2"));
...
const englishLessons = [...lessons.filter((lesson) => lesson.subject === "english"), ...extendedEnglishCourse];
...
const WritingStudio = dynamic(() => import("./writing-area"), { loading: () => <div className="module-loading">Preparando seu espaço de redação…</div> });
const EnglishWorkspace = dynamic(() => import("./english-workspace"), { loading: () => <div className="module-loading">Preparando suas aulas de inglês…</div> });
...
export default function StudyDashboard() {
  ...
  useEffect(() => {
    const timer = window.setTimeout(() => {
      try {
        ...
        setSkillProgress(onlyEnemSkills(JSON.parse(window.localStorage.getItem("lord-enem-progress") ?? "{}")));
  ...
  useEffect(() => onAuthStateChanged(firebaseAuth, async (currentUser) => {
    ...
    try {
      ...
      if (resetToken > previousResetToken) {
        [`clareia-english-v2:${currentUser.uid}`, "lord-focus-progress", "lord-focus-tasks", "lord-enem-progress", "lord-enem-assessments", "clareia-exam-answer-sheets", "lord-question-progress", "clareia-essays"].forEach((key) => window.localStorage.removeItem(key));
      ...
      if (cloudSnapshot.exists()) {
        ...
        if (cloud.skillProgress) { const enemProgress = onlyEnemSkills(cloud.skillProgress); setSkillProgress(enemProgress); window.localStorage.setItem("lord-enem-progress", JSON.stringify(enemProgress)); }
  ...
  function toggleSkillStage(skill: SkillWithId, stage: Stage) {
    const next = { ...skillProgress, [skill.id]: { ...skillProgress[skill.id], [stage]: !skillProgress[skill.id]?.[stage] } }; setSkillProgress(next); window.localStorage.setItem("lord-enem-progress", JSON.stringify(next)); void saveToCloud({ skillProgress: { [skill.id]: next[skill.id] } }); setNotice(`${stages.find((item) => item.id === stage)?.label} atualizada em ${displayTopic(skill)}.`);
  }
  ...
  return <main className="app-shell">
    ...
    <section className="workspace"><header className="topbar"><button type="button" className="menu-button" aria-label={mobileMenu ? "Fechar menu" : "Abrir menu"} aria-controls="main-sidebar" aria-expanded={mobileMenu} onClick={() => setMobileMenu((value) => !value)}>☰</button><div><p>{formatDate(currentDate)}</p><h1>{viewLabels[view]}</h1></div><div className="top-actions"><div className="academic-year-chip"><span>ENEM</span><small>{currentDate?.getFullYear() ?? ""}</small></div><button className="quick-add" onClick={() => setView("tasks")}>+ Planejar</button><button className={`sync-account ${syncState}`} onClick={disconnectGoogle} title="Clique para sair da conta"><span>{syncState === "syncing" ? "↻" : syncState === "error" ? "!" : "✓"}</span><div><strong>{syncState === "syncing" ? "Salvando..." : syncState === "error" ? "Só neste aparelho" : "Sincronizado"}</strong><small>{user.displayName?.split(" ")[0] ?? user.email}</small></div></button></div></header>
      ...
      {view === "english" && user && <EnglishWorkspace key={user.uid} uid={user.uid} legacyProgress={progress} onComplete={(lesson) => setLessonStatus(lesson, "done")} />}
