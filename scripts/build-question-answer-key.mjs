...
function parseProof(text, year, keyMap, sourcePrefix) {
  ...
  for (let index = 0; index < matches.length; index += 1) {
    ...
    const area = number >= 136 && number <= 180 ? "MT" : year <= 2016 && number >= 46 && number <= 90 ? "CN" : year >= 2017 && number >= 91 && number <= 135 ? "CN" : "";
...
for (const [groupKey, groupQuestions] of groups) {
  ...
  if (!ppl) {
    ...
    candidates = (exam.questions ?? []).filter((item) => !item.cancelled && /^[A-E]$/.test(item.correctAlternative ?? "") && (area === "MT" ? Number(item.index) >= 136 && Number(item.index) <= 180 : year <= 2016 ? Number(item.index) >= 46 && Number(item.index) <= 90 : Number(item.index) >= 91 && Number(item.index) <= 135)).map((item) => ({
